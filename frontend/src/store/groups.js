  ...
  switch (action.type) {
    case LOAD_GROUPS:
      ...
      action.payload.Groups.forEach(
        (group) => (newState.allGroups[group.id] = group)
      );
      return newState;
    case GET_GROUP:
    ...
    case GET_MEMBERS:
      ...
      action.payload.Members.forEach(
        (member) => (newState.allMembers[member.id] = member)
      );
      return newState;
    case ADD_MEMBER:
    ...
    case GET_EVENTS:
      ...
      action.payload.Events.forEach(
        (event) => (newState.allEvents[event.id] = event)
      );
      return newState;
    default:
      return state;
